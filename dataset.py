...
_ds2 = load_dataset("mteb/sts12-sts", split="test").map(normalize_score, fn_kwargs={"_min": 0.0, "_max": 5.0})
_ds3 = load_dataset("SemRel/SemRel2024", name="eng", split="train").rename_column(
    "label", SCORE
)
_ds4 = load_dataset("mteb/sickr-sts", split="test").map(normalize_score, fn_kwargs={"_min": 1.0, "_max": 5.0})
